		...
		<view class="lists">
			<view class="wallet-list" v-for="(item, index) in orderList" :key="index" @click="goTo(item)">
				<view class="oneOrder">
					...
					<view class="btn" v-if="item.status == 0 || (item.status > 0 && item < 5 && item.payStatus)">
						<view v-if="item.status == 0" @click.stop="cancelOrder(item)">取消订单</view>
						<view v-if="item.status > 0 && item < 5 && item.payStatus" @click.stop="confirmReceipt(item)">确认订单</view>
					</view>
				</view>
			</view>
